  ...
  override def reconcileWith(appId: PathId): Future[Unit] =
    appRepository.currentVersion(appId) flatMap {
      ...
      case Some(app) =>
        ...
        val activeVersions: Set[String] = tasks.iterator.map(_.getVersion).toSet + app.version.toString

        val ahcs = appHealthChecks.writeLock { ahcs =>
          ...
          for {
            (version, activeHealthChecks) <- ahcs(appId)
            if version != app.version && !activeVersions.contains(version.toString)
            activeHealthCheck <- activeHealthChecks
          } remove(appId, version, activeHealthCheck.healthCheck)
        ...
        val healthCheckVersions: Set[String] = ahcs(appId).iterator.map(_._1.toString).toSet
        val newVersions = activeVersions -- healthCheckVersions
        val res = newVersions map { version =>
